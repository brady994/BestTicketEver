...
public class UserService {
	...
	public JsonObject deleteWishlist(String json) {
		JsonObject result = new JsonObject();
		DAOFactory postgres = DAOFactory.getDaoFactory(DAOFactory.POSTGRES);
		WishlistDAO wishdao = postgres.getWishlistDAO();
		Gson gson = new Gson();
		Wishlist wish = gson.fromJson(json, Wishlist.class);
		if (wishdao.delete(wish)>0)
		{
			result.addProperty("result", "SUCCESS");
			result.addProperty("message", "The wishlist has been deleted");
		}else{
			result.addProperty("result", "FAIL");
			result.addProperty("reason", "Something weird happend, we are sorry");
		}
	...
	public JsonObject deleteTicketWishList(String json) {
		JsonObject result = new JsonObject();
		DAOFactory postgres = DAOFactory.getDaoFactory(DAOFactory.POSTGRES);
		WishlistDAO wishdao = postgres.getWishlistDAO();
		Gson gson = new Gson();
