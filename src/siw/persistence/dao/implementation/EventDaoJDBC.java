...
public class EventDaoJDBC implements EventDAO {
	...
	try {
		connection = datasource.getConnection();
		query = "SELECT e.name as nameE, e.idevent, e.date, e.description, e.location, e.image, g.name, g.idguest FROM Event AS e INNER JOIN event_has_guest AS eg ON eg.event_idevent=e.idevent INNER JOIN guest AS g ON g.idguest=eg.guest_idguest where idevent=?";
		statement = connection.prepareStatement(query);
